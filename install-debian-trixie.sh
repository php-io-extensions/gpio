...
# Installer for the GPIO extension on Debian Trixie (and compatible Ubuntu).
...
header() {
    echo "=========================================="
    echo " GPIO Extension Installer (Debian Trixie)"
    echo "=========================================="
    echo ""
}
...
# Ensure <linux/gpio.h> is available (linux-libc-dev).
if ! dpkg -l linux-libc-dev >/dev/null 2>&1 || \
   ! [ -f /usr/include/linux/gpio.h ]; then
